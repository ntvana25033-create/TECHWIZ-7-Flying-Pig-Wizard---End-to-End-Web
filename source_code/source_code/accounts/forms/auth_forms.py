...
class RegisterForm(FormStyleMixin, forms.Form):
    ...
    accept_terms = forms.BooleanField(
        label="I agree to the terms of use and privacy policy", required=True
    )
    ...
    def clean(self):
        ...
        if email and password:
            ...
            if self.user_cache is None:
                raise ValidationError("Incorrect email or password, or the account is locked")
        return cleaned
...
class ResetPasswordForm(FormStyleMixin, forms.Form):
    ...
    def clean(self):
        ...
        if password1 and password2 and password1 != password2:
            self.add_error("new_password2", "The password confirmation does not match")
        if password1:
            try:
                password_validation.validate_password(password1)
            except ValidationError as exc:
